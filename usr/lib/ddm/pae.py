...
class PAE():
    ...
    def getInstalledPaePackage(self):
        ...
        if 'pae' in self.kernelRelease:
            ...
            if functions.isPackageInstalled(linHeaders):
                ...
                self.log.write('Installed PAE: %s' % installedPae, 'pae.getInstalledPaePackage', 'info')
        return installedPae

    # Check if the PAE kernel can be installed
    def getPae(self):
        hwList = []
        description = 'Multi-core support for 32-bit systems'
        self.log.write('Distribution: %s' % self.distribution + ' ' + str(self.distributionReleaseNumber), 'pae.getPae', 'debug')
        ...
        if not self.needCheckForPae():
            ...
            if installedPackage:
                ...
                self.log.write('Multi-core already installed: %s' % installedPackage, 'pae.getPae', 'info')
                hwList.append(['PAE capable system', hwCodes[5], packageStatus[0], installedPackage, version, description])
            else:
                self.log.write('Single-core kernel found: %s' % self.kernelRelease, 'pae.getPae', 'debug')
                ...
                if machine[0] == 'i686':
                    ...
                    for package in self.packages:
                        if not functions.isPackageInstalled(package):
                            ...
                            hwList.append(['PAE capable system', hwCodes[5], status, package, version, description])
                            break
    ...
    def installPAE(self):
        try:
            ...
            for package in self.packages:
                cmdPae += ' ' + package
            self.log.write('PAE kernel install command: %s' % cmdPae, 'pae.installPAE', 'debug')
            self.ec.run(cmdPae)
        ...
        try:
            ...
            if not 'pae' in kernelRelease[0]:
                ...
                for package in self.packages:
                    ...
                    self.log.write('PAE package to remove: %s' % package, 'pae.removePAE', 'info')
