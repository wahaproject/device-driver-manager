...
#       MA 02110-1301, USA.
#
#       [Schoelje, 3 December 2012]
#       Adapted for use with DDM (Device Driver Manager).
#       selectDrivers will return a list with possible drivers for present cards.
#       This includes the new experimental packages and the update packages.
...
class NvidiaDetection(object):
    ...
    def __init__(self, printonly=None, verbose=None, obsolete=obsoletePackagesPath):
        ...
        self.__driver_aliases = {'current': 1000, 'experimental-310': 999, 'experimental-304': 998}
    ...
    def __get_value_from_name(self, name):
        ...
        #print '__get_value_from_name = %s' % name
    ...
    def getData(self):
        ...
        for package in apt.Cache():
            ...
            if not package.name.startswith('nvidia-'):
                continue
            elif 'updates' in package.name and not '-dev' in package.name and not 'settings' in package.name:
                self.updates.append(package.name)
                continue
            try:
                # As long as the Debian driver packages do not include modaliases, this only works in Ubuntu
        ...
        if cardsNumber > 0:    # if a NVIDIA card is available
            if cardsNumber > 1:    # if more than 1 card
                ...
                if len(candidates) > 0:
                    ...
                    if self.verbose and not self.printonly:
                        print 'Recommended NVIDIA drivers:', str(choices)
                else:
                    '''
                ...
                '''
                choices = self.driversForCards[self.driversForCards.keys()[0]]
            ...
            for choice in choices:
                ...
                if driver_name is not None:
                    ...
                    supportedDrivers.append('nvidia-' + str(driver_name))
                else:
                    #print 'choice=%s' % choice
                    supportedDrivers.append('nvidia-' + str(choice))

                if len(supportedDrivers) == 1:
                    if supportedDrivers[0] + '-updates' in self.updates:
                        supportedDrivers.append(supportedDrivers[0] + '-updates')
    ...
    def checkpkg(self, pkglist):
        ...
        if self.isstr(pkglist):    # if it is a string
            try:
    ...
    def printSelection(self):
        ...
        '''
        pass
        drivers = self.selectDrivers()
        if self.getDrivers():    # if an old driver is installed
            if drivers:    # if an appropriate driver is found
                sys.stdout.flush()
                print str(drivers)
            else:
                sys.stdout.flush()
